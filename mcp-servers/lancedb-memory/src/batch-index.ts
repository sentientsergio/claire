...
interface IndexState {
  lastIndexedFiles: string[];     // daily memory files already indexed
  lastTranscriptHash: string;     // hash of last indexed transcript content
  lastRunAt: string;
}
...
function chunkDailyMemory(filename: string, content: string): Chunk[] {
...
async function main() {
  ...
  if (existsSync(memoryDir)) {
    ...
    for (const file of files) {
      if (!force && state.lastIndexedFiles.includes(file)) continue;
